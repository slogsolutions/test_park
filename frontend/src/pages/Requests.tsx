...
//     // Remove booking from UI list (it's deleted from DB)
//     setBookings(prev => prev.filter((b: any) => (b._id || b.id) !== bookingId));
//     alert("Parking session completed and booking closed!");
//     setSecondOtpInput("");
//   } catch (err) {
...
//             console.log("DEBUG if Current Stable ID:", stableId, "Verifying Second OTP:", verifyingSecondOtp);
//             setSecondOtpInput(e.target.value.replace(/\D/g, '').slice(0, 6));
//           } else {
//             console.log(" DEBUG else Current Stable ID:", stableId, "Verifying Second OTP:", verifyingSecondOtp);
...
const ProviderBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        const data = await response.json();
        // backend may return { bookings: [...] } or raw array; handle both
  ...
  const verifyOtpForBooking = async (bookingId: string) => {
    ...
    try {
      ...
      alert('OTP verified — booking confirmed!');
      // Update local booking status to match backend (use returned booking if present)
  ...
  // SECOND OTP: mark booking completed instead of removing it
  const verifySecondOtpForBooking = async (bookingId: string) => {
    if (!secondOtpInput.trim() || secondOtpInput.length !== 6) {
      alert("Please enter a valid 6-digit second OTP");
      return;
    ...
    try {
      ...
      // Instead of removing the booking from UI (previous behavior when backend deleted),
      // update the booking in-place with the returned booking (or at least status: 'completed').
      const returned = data.booking || data;
      setBookings(prev => prev.map((b: any) => {
        const stable = b._id || b.id;
        if (stable === bookingId) {
          // merge returned fields if present
            ...
            return (
              <motion.div key={stableId} layout className="p-6 border rounded-xl">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <p className="text-lg font-semibold">₹{booking.totalPrice ? Math.ceil(booking.totalPrice) : "N/A"}</p>
                    <p className={`text-sm font-medium ${booking.status === 'accepted' ? 'text-green-600' : booking.status === 'rejected' ? 'text-red-600' : booking.status === 'confirmed' ? 'text-blue-600' : booking.status === 'completed' ? 'text-gray-600' : 'text-yellow-600'}`}>
                      {booking.status ? (booking.status.charAt(0).toUpperCase() + booking.status.slice(1)) : 'Unknown'}
                    </p>
                    {booking.paymentStatus && (
                      <p className={`text-xs ${booking.paymentStatus === 'paid' ? 'text-green-500' : 'text-orange-500'}`}>

...
const ProviderBookings = () => {
  ...
  const [verifyingSecondOtp, setVerifyingSecondOtp] = useState<string | null>(
    null
  );
  ...
  // returns true if current time is between 2 hours and 10 hours before startTime
  // helper: return true if current time is between 2 hours and 10 hours before startTime
  // const canCancelBooking = (booking: Booking) => {
  //   if (!booking.startTime) return false;
  //   const startTs = new Date(booking.startTime).getTime();
  //   if (isNaN(startTs)) return false;
  //   const now = Date.now();
  //   const twoHoursBefore = startTs - 2 * 60 * 60 * 1000;
  //   const tenHoursBefore = startTs - 10 * 60 * 60 * 1000;
  //   return now >= tenHoursBefore && now <= twoHoursBefore;
  ...
  // Only allow cancellation if current time is earlier than 24 hours before start time
        ...
        prevBookings.map((booking: any) =>
          booking._id === bookingId
            ? {
                ...booking,
                status: newStatus,
                providerId: updatedBooking?.providerId || booking.providerId,
              }
            : booking
        )
      );
  ...
  const verifySecondOtpForBooking = async (bookingId: string) => {
    ...
    try {
      ...
      const res = await fetch(
        ...
        }/api/booking/${bookingId}/verify-second-otp`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleReject = (bookingId: string) => {
    ...
    if (!canRejectBooking(booking)) {
      alert(
        "You can only reject a booking earlier than 1 hour before the start time."
      );
      return;
    }
  ...
  const confirmReject = () => {
    if (selectedBooking) {
      ...
      const booking = bookings.find(
        (b) => b._id === selectedBooking || b.id === selectedBooking
      );
                    ...
                    {canCancelBooking(booking) &&
                      ...
                      booking.status !== "cancelled" && (
                        <button
                          ...
                          className="flex-1 bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg transition-colors duration-200"
                        >
                          Cancel Booking
                        </button>
                      )}
                      ...
                      {canEnterOtp(booking) ? (
                        <div>
                          ...
                          <p className="text-sm text-blue-600 mb-3">
                            Ask the customer for their OTP to confirm their
                            arrival and start the session.
                          </p>
                        ...
                        <div className="bg-yellow-50 rounded-lg p-3 border-yellow-100">
                          <div className="flex items-center mb-2">
                            ...
                            <h4 className="text-sm font-semibold text-yellow-800">
                              OTP locked until start time
                            </h4>
                          </div>
                          <p className="text-sm text-yellow-700">
                            OTP entry is allowed only after the booking start
                            time. {timeUntilStartText(booking)}.
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
                    <div className="flex items-center mb-3">
                      ...
                      <h4 className="text-sm font-semibold text-green-800">
                        End Session - Second OTP
                      </h4>
                    </div>
                    <p className="text-sm text-green-600 mb-3">
                      When the customer is leaving, ask them for their second
                      OTP to complete and close the booking.
                    </p>
                    <div className="flex space-x-3">
                      <input
                        type="text"
                        ...
                        value={
                          verifyingSecondOtp === stableId ? secondOtpInput : ""
                        }
                        onChange={(e) => {

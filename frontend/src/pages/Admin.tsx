...
export default function AdminDashboard() {
    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          err.message ||
          "Failed to fetch parking spaces"
      );
    } finally {
      setLoading(false);
    }
    ...
    const fetchDashboardStats = async () => {
      try {
        ...
        const activeBookings = allBookings.filter((booking) =>
          ["confirmed", "pending", "accepted"].includes(
  ...
  const generateRecentActivity = () => {
    ...
    recentBookings.forEach((booking, index) => {
      activities.push({
        type: "booking",
        message: `${booking.user?.name || "User"} booked ${
  ...
  const UserEditModal = ({ user, onClose, onSave }) => {
    ...
    const handleVerify = async () => {
      try {
        await api.patch(`/admin/users/${user._id}/verify`, {
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-2xl p-6 w-full max-w-md">
          ...
          <div className="mt-4">
            ...
            <select
              onChange={(e) => handleKYCUpdate(e.target.value)}
  ...
  const SpaceEditModal = ({ space, onClose, onSave }) => {
    ...
    const handleDelete = async () => {
      if (
        window.confirm("Are you sure you want to delete this parking space?")
      ) {
        try {
  ...
  const renderContent = () => {
    ...
    switch (tab) {
      ...
      case "users":
        const uniqueStates = [
          ...new Set(users.map((u) => u.kycData?.state || "").filter(Boolean)),
        ].sort();
        const uniqueCities = [
          ...new Set(users.map((u) => u.kycData?.city || "").filter(Boolean)),
        ].sort();
        const filteredUsers = users.filter((user) => {
          let match = true;
          if (isVerifiedFilter !== null)
            match = match && user.isVerified === isVerifiedFilter;
          if (kycStatusFilter !== null)
            match = match && user.kycStatus === kycStatusFilter;
          if (stateFilter !== null)
            match = match && user.kycData?.state === stateFilter;
          if (cityFilter !== null)
            match = match && user.kycData?.city === cityFilter;
          return match;
        });
        return (
          <div>
      ...
      case "bookings":
        ...
        const uniqueBookingCities = [
          ...new Set(
            bookings
              .map((b) => b.parkingSpace?.address?.city || "")
              .filter(Boolean)
          ),
        ].sort();
        const filteredBookings = bookings.filter((booking) => {
          ...
          if (bookingStatusFilter !== null)
            match = match && booking.status === bookingStatusFilter;
          if (bookingCityFilter !== null)
            match =
              match &&
              booking.parkingSpace?.address?.city === bookingCityFilter;
          return match;
        });
        return (
          <div>
                ...
                },
                {
                  ...
                  render: (booking) => (
                    <div className="text-sm">
                      ...
                      <div className="text-gray-500">
                        {booking.parkingSpace?.address?.city || "No location"}
                      </div>
                    </div>
                  ),
                },
      ...
      case "parkingspaces":
        const uniqueSpaceStatuses = [
          ...new Set(spaces.map((s) => s.status).filter(Boolean)),
        ].sort();
        const uniqueSpaceStates = [
          ...new Set(spaces.map((s) => s.address?.state || "").filter(Boolean)),
        ].sort();
        const uniqueSpaceCities = [
          ...new Set(spaces.map((s) => s.address?.city || "").filter(Boolean)),
        ].sort();
        const filteredSpaces = spaces.filter((space) => {
          let match = true;
          if (spaceStatusFilter !== null)
            match = match && space.status === spaceStatusFilter;
          if (spaceStateFilter !== null)
            match = match && space.address?.state === spaceStateFilter;
          if (spaceCityFilter !== null)
            match = match && space.address?.city === spaceCityFilter;
          return match;
        });
        return (
          <div>
        ...
        // here added Notification added component
         case "notification":
        return (<Notification />);
        default:
        return <DashboardContent />;
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div
        ...
        } transition-all duration-300 ease-in-out bg-white shadow-2xl border-r border-gray-100`}
      >
            ...
            <div className="space-y-2">
              {sidebarItems.map((item) => (
                ...
                >
                  <span
                    className={`${
                      tab === item.id ? "text-white" : "text-gray-400"
                    }`}
                  >
                    {item.icon}
                  </span>
